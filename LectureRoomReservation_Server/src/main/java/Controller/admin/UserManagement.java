...
public class UserManagement implements Admin {
    ...
    // 사용자 추가 기능
    public boolean addUser(List<String> newUser) { // 매개변수로 추가할 사용자의 정보를 받음
        String newId = newUser.get(this.userIdIdx); // 추가할 사용자의 id를 따로 저장
        
        if(this.getIdCheck(newId)) {    // 위에서 따로 저장한 id로 이미 존재하는 id인지 판별
            System.out.println("이미 존재하는 아이디입니다.");
            return false;
        }
        
        this.userInfo.add(newUser); // 존재하지 않는 id라면 추가
        
        try {
            this.excel.saveUserInfo(this.userInfo); // 사용자 정보를 추가
            System.out.println("저장 완료");
            return true;
        } catch (Exception e) {
            System.out.println("예외 발생: " + e.getMessage());
